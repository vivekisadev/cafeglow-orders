...
const isDeveloper = (req, res, next) => {
    // In development, allow access with a simple header or query param
    // In production, implement proper developer authentication
...
router.get('/developer/admins', isDeveloper, async (req, res) => {
    try {
        const admins = await adminModel.find({}, { password: 0 }); // Don't send passwords
...
router.post('/developer/admins/create', isDeveloper, async (req, res) => {
    try {
...
router.delete('/developer/admins/:id', isDeveloper, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/developer/admins/reset', isDeveloper, async (req, res) => {
    try {
        const { name, email, password } = req.body;
